...
public:
    ...
    void runTest() override
    {
        ...
        {
            expect (WhatsNew::shouldShow ("0.1.0", "0.3.0"));
            expect (WhatsNew::shouldShow ("0.3.0", "0.1.0"), "a downgrade should still surface that version's notes");
        }

        beginTest ("releaseNotes has a well-formed entry for the current 0.3.0 release");
        {
            ...
            {
                if (entry.first != "0.3.0")
                    continue;

                found = true;
                expect (! entry.second.isEmpty(), "0.3.0 must ship with at least one bullet point");
            }
